...
public:
  MainV3 *mainBoard;
  Encoder<TimerEnc1> encX;
  Encoder<TimerEnc2> encY;
  ...
  SensorTimer(MainV3 *_mainBoard) : encX(), encY() {
...
int main(void)
{
  ...
  std_msgs::Int32 encoderX_msg;
  encoderX_msg.data = 0;
  ros::Publisher encoderX("encX", &encoderX_msg);
  nh.advertise(encoderX);

  std_msgs::Int32 encoderY_msg;
  encoderY_msg.data = 0;
  ros::Publisher encoderY("encY", &encoderY_msg);
  nh.advertise(encoderY);
