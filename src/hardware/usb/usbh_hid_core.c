                                           ...
                                           void *phost)
{
  ...
  if(pphost->device_prop.Itf_Desc[0].bInterfaceSubClass  == PS3_BOOT_CODE)
  {
	  /*Decode Bootclass Protocl: Mouse or Keyboard*/
	  if(pphost->device_prop.Itf_Desc[0].bInterfaceProtocol == HID_PS3_BOOT_CODE)
	  {
		  HID_Machine.cb = &HID_PS3_cb;
	  }
                                         ...
                                         void *phost)
{
  ...
  case HID_REQ_GET_REPORT_DESC:
    ...
    {
    	HID_Machine.ctl_state = HID_PS3_BOOTCODE;
    }
  ...
  case HID_PS3_BOOTCODE:
  {
	  ...
	  uint8_t reportID = 0xF4;
	  USBH_Status s = USBH_Set_Report(pdev, pphost, USB_REQ_SET_FEATURE, reportID, 4, cmd_buf);
